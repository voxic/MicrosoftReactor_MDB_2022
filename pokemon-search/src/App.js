...
function App() {
  ...
  return (
    ...
    <Grid container spacing={2}>
      ...
      <Grid md={12} lg={12}>
        <TextField sx={{width: "100%", pb: 3}} inputRef={searchInputRef} onChange={handleSearch} on id="outlined-search" label="Search field" type="search" />
        <Divider />
      </Grid>
      { searching && 
      ...
      { pokemons.length > 0 && 
        pokemons.map((pokemon) => {
          console.log(pokemon)
            return(<Grid>
              <PokemonCard id={pokemon.id} name={pokemon.name} species={pokemon.species.name} sprite={pokemon.sprite} />
            </Grid>)
          })}
      { pokemons.length < 1 && 
          <Typography variant="h4" gutterBottom>
